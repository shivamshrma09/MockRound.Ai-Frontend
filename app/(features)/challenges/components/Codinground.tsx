...
import {getquestions } from "../services/getquestions.services";
...
function CodingEnvironment({ questionsList, testdetails, onBack }) {
  ...
  const [timer, setTimer] = useState<number>(140);
  ...
  useEffect(() => {
    const verifyAccess = async () => {
      try {
        ...
        const userId = params?.userId || params?.id || params?.[3];
        ...
        let participationResult = await checkParticipated(challengeId, userId);
    ...
    const fetchQuestions = async () => {
      if (!questionsList || questionsList.length === 0) {
        try {
          ...
          setLocalQuestionsList(qList);
          if (qList && qList[0]?.time) {
            setTimer(qList[0].time);
          }
        } catch (err) {
      ...
      <div className="border-b border-[#191919] h-12 px-4">
        <div className="flex items-center h-full">
          <div className="flex items-center gap-2 text-white">
            ...
            <span>{minute + "hr" + " " + second + "min" + " "}</span>
                ...
                {selectedTestCase && getCurrentTestResults().length > 0 && (
                  <div className=" rounded-lg p-4 border border-gray-700">
                    {(() => {
                      const currentResults = getCurrentTestResults();
                      const result = currentResults.find(
                        (r) => r.caseNumber === selectedTestCase
                      );
                      if (!result)
...
onClick={async () => {
  ...
  try {
    let roundNumber = 1;
    let challengeId = process.env.NEXT_PUBLIC_CHALLENGE_ID
