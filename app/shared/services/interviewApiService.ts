...
export const interviewApiService = {
  getRoundConfig: (roundType: string) => {
    const configs = {
      'HR': { maxQuestions: 3, endpoint: '/api/hr' },
      'Technical': { maxQuestions: 6, endpoint: '/api/technical' },
      'Coding': { maxQuestions: 3, endpoint: '/api/coding' },
      'Behavioral': { maxQuestions: 4, endpoint: '/api/behavioral' }
    };
  ...
  prepareRequestData: (
    ...
    if (roundType === 'Coding' && code) {
      ...
      requestData.language = language || 'JavaScript';
      requestData.explanation = explanation || '';
            requestData.previous_answer = `
Language: ${language}
Code:
${code}
